...
class User extends CI_Controller 
{
	...
	public function index($keyword = 'null')
	{
		...
		$config['base_url'] = base_url().'user/index/' . $keywordDuplicate . '/';
		...
		$result["users"] = $this->M_user->getUsers($start,$limit,$keyword,$filter=null);	
		//echo "<pre>";
		//print_r($result["users"]);	
		//echo "</pre>";
		$result['pageHeading'] = 'Manage Universities';
	...
	public function exportData()
		{
			$users = $this->M_user->getUsers($start=null,$limit=null,$keyword=null,$filter=null);	
			
			foreach ($users as $key => $value) {
				$tempUserData['ID']=$value['id'];
				$tempUserData['Name']=$value['name'];
				$tempUserData['Website Url']=$value['domain'];
				$tempUserData['Status']=($value['is_verified']=='Y' ? 'Verified' : 'Pending');
				$userData[]=$tempUserData;
			}

			 $this->load->helper('exportData');
			 $fileName = "User_export_data" . date('Ymd') . ".xls";
	...
	public function changeVerified()
	{
		...
		if(($check['is_verified'] == 'N' || $check['is_verified'] == 'P') &&  $data['is_verified']=='Y'){
		    ...
		    $header .= 'From: DirectParking<noreply@directparking.com>' . "\r\n";
			$body = "Dear Customer,
			...
			";
			$subject = "Account Varification: DirectParking";
	...
	public function view()
	{
		...
		$this->load->view('v_header');
		$this->load->view('user/v_userView',$catResult);
		$this->load->view('v_footer');	
	}
