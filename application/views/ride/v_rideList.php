...
<div class="table_data" aria-labelledby="headingOne" role="tabpanel" id="chat">
	...
	<div class="portlet box grey">
						<div class="portlet-title">
							...
							<a class="btn blue" href="<?php echo site_url(); ?>ride/exportData">Export</a>
							</div>
							<!--div class="actions">
					...
					<?php }?>
								...
								<td>
									...
									<?php } elseif($row['status'] == 'accept'){ ?>
										<div class="btn-success">&nbsp;&nbsp; Accept</div>
									<?php } ?>
...
<!-- 						<a href="<?php echo site_url(); ?>user/edit?id=<?php echo base64_encode($row['id']); ?>" class="btn default btn-full" style="background:#E8720B;color:#FFF;"><i class="fa fa-pencil"></i> Edit</a> -->
					...
					<?php /*if($row['enabled'] == 'Y'){?>
						<a id="rowIdStatus_<?php echo $row['id']; ?>" href="javascript:void(0)" onclick="changeStatus(<?php echo $row['id']; ?>, 'N')"><button type="button" class="btn btn-warning btn-full"> Dactive</button></a>
					<?php }else{?>
						<a id="rowIdStatus_<?php echo $row['id']; ?>" href="javascript:void(0)" onclick="changeStatus(<?php echo $row['id']; ?>, 'Y')"><button type="button" class="btn btn-primary btn-full"> Active</button></a>
					<?php }*/?>
			  ...
			  if(data == 'true')
			  {
				  ...
				  { 
					  /*$('#rowId_'+gId +'td:nth-child(3) a:nth-child(3)').html('Disable');*/ 
					   $('#rowIdStatus_'+id).html('<button type="button" class="btn btn-warning"><i class="fa fa-eye"></i> Dactive </button>');
				  	   $('#rowIdStatus_'+id).attr("onclick", "changeStatus("+id+", 'N')");
				  }
				  else
				  {
					  /*$('#rowId_'+gId +'td:nth-child(3) a:nth-child(3)').html('Enable');*/
					  $('#rowIdStatus_'+id).html('<button type="button" class="btn btn-info"><i class="fa fa-eye"></i> Active </button>');
...
alert('Your search string contains illegal characters.');
 ...
 window.location.href = window.location.href='<?php echo base_url().'ride/'.$pageFunction.'/'; ?>' + $('#searchFORMHEAD_input').val()+ '<?php echo '?'.$_SERVER['QUERY_STRING'];?>';
