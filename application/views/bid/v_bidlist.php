	...
	<div class="portlet box grey">
						...
						<div>
							...
							<tr class="odd gradeX"  id="rowId_<?php echo $row['id']; ?>">
								...
								<td><?php if($row['created'] != "0000-00-00 00:00:00"){echo date('d M,Y',strtotime($row['created']));} ?></td>
								<td>
...
function search() {
	...
	if(/^[a-zA-Z0-9- ]*$/.test(search_btn) == false) {
		...
		window.location.href = window.location.href='<?php echo base_url().'bid/index/'; ?>' + $('#searchFORMHEAD_input').val();
	}
}
function deleted(id)
	{
		...
		if (confirm("Are you sure, you want to delete?") == true) {
			...
			$.ajax({
				...
				console.log(data);
				  ...
				  { 
					  $('#message').html('<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Failed!</strong> could not deleted.</div>');
