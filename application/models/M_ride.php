...
class m_ride extends CI_Model {
	...
	public function countRide($keyword,$filter)
	{
		$this->db->select('*');
		$this->db->from('ride_booking as R');
		$this->db->join('tbl_users as U','U.id = R.user_id');
		$this->db->where("(U.fname LIKE '%$keyword%' or U.email LIKE '%$keyword%' or U.mobile LIKE '%$keyword%') ");
	...
	public function getRides($start,$limit,$keyword,$filter)
	{

		$this->db->select('U.*,UV.name as university_name,R.*');		
		$this->db->from('ride_booking as R');
		$this->db->join('tbl_users as U','U.id = R.user_id');
		$this->db->join('universities as UV','UV.id = U.university_id');
		$this->db->where("(U.fname LIKE '%$keyword%' or U.email LIKE '%$keyword%' or U.mobile LIKE '%$keyword%') ");
		
		if(isset($filter['U.is_verified'])){
			$this->db->where('U.is_verified',$filter['U.is_verified']);
		}
		
		if(isset($filter['date_from']) && isset($filter['date_to'])){
			$this->db->where('U.created >=',$filter['date_from']);
			$this->db->where('U.created <=',$filter['date_to']);
		}
        if(!empty($filter['status'])){
        	$this->db->where('U.is_verified',$filter['status']);
        }
	...
	{
		//print_r($id);die;
		$this->db->select('U.*, UV.name as university_name');
		$this->db->from('tbl_users as U');
		$this->db->join('universities as UV','U.university_id = UV.id','left');
