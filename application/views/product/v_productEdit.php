								...
								<div class="actions btn-set">
									...
									<div class="btn-group">
										...
										<ul class="dropdown-menu pull-right">
											...
											<li>
												<a href="<?php echo site_url(); ?>product/copy?pid=<?php echo base64_encode($productData['id']);?>">
													Copy Product
												</a>
											</li>
								...
								<div class="tabbable">
									...
									<div class="tab-content no-space">
										...
										<div class="tab-pane" id="tab_graphs">
											...
											<div id="tab_graphs_uploader_container" class="text-align-reverse margin-bottom-10">
											<input type="file" name="graph[]" style="display:inline !important;" class="btn yellow" multiple>												<!--a id="tab_images_uploader_pickfiles" href="javascript:;" class="btn yellow">
													<i class="fa fa-plus"></i> Select Files
												</a-->
												<a href="javascript:uploadGraph();" class="btn green">
													<i class="fa fa-share"></i> Upload Files
												</a>
											</div>
											<div class="row">
											...
											<tbody id="tblGraphBody">

											<?php 
											if(!empty($graphImages)){
											foreach($graphImages as $images){?>
											<tr id="rowGraph_<?php echo $images['id'];?>">
												<td>
													<a href="<?php echo site_url().'assets/image/graphs/original/'.$images['image']; ?>" class="fancybox-button" data-rel="fancybox-button">
														<img width="150" src="<?php echo site_url().'assets/image/graphs/original/'.$images['image']; ?>" alt="">
													</a>
												</td>
												
												<td>
													<a href="javascript:;" onclick="removeGraph(<?php echo $images['id'];?>)" class="btn default btn-sm">
														<i class="fa fa-times"></i> Remove
													</a>
												</td>
											</tr>
...
function uploadGraph()
{
	...
	$.ajax({
		...
		success: function (results)
		{
			...
			if(results == 'false')
			{
				$('#message').html('<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Danger!</strong> graphs not uploaded.</div>');
			}
			else
			{
				/*$('#message').html('<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Success!</strong> images uploaded.</div>');*/
				...
				$.each(results, function(k, v){
        			...
        			htmlStr += '<tr><td><a href="<?php echo site_url()?>assets/image/graphs/original/'+v.image+'" class="fancybox-button" data-rel="fancybox-button"><img width="150" src="<?php echo site_url()?>assets/image/graphs/original/'+v.image+'" alt=""><td><a href="javascript:;" onclick="removeGraph('+v.id+')" class="btn default btn-sm"><i class="fa fa-times"></i> Remove</a></td></tr>';
   				});
   				$('#tblGraphBody').html(htmlStr);
			...
			$.ajax({
				...
				console.log(data);
				  if(data == 'true')
				  {
					  ...
					  $('#message').html('<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Success!</strong> graph deleted.</div>');
					   $('#rowGraph_'+id).remove();
				  }
				  else
				  { 
					  $('#message').html('<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Failed!</strong> graph not deleted.</div>');
