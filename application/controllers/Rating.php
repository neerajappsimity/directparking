...
class Rating extends CI_Controller 
{
	...
	public function index($keyword = 'null')
	{
		...
		$config['base_url'] = base_url().'rating/index/' . $keywordDuplicate . '/';
		
		$count = $this->M_user->countUsers($keyword,$filter=null);
		...
		$result["users"] = $this->M_rating->getRating($start,$limit,$keyword,$filter=null);	
		/*echo "<pre>";
		print_r($result["users"]);	
		echo "</pre>";*/
		$result['pageHeading'] = 'Manage Rating';
		$result['startPage'] = $start;
		$result['pageFunction'] = 'index';
				
		$this->load->view('v_header');
		$this->load->view('rating/v_ratinglist',$result);
		$this->load->view('v_footer');
	}
	...
	public function changeStatus()
	{
		...
		$result = $this->M_user->changeStatus($data);
		echo $result;
	}
	...
	public function changeVerified()
	{
		$data = $_POST;
		//print_r($data);die;
		//$check = $this->M_rating->checkVerified($data);
		//print_r($check);
	/*	if(($check['is_verified'] == 'N' || $check['is_verified'] == 'P') &&  $data['is_verified']=='Y'){
		...
		$result = $this->M_rating->changeVerified($data);
		echo $result;
	}
	...
	public function deleted()
	{
		...
		$result = $this->M_user->deleted($data);
		echo $result;
	}
	...
	public function view()
	{
		...
		$result['userData'] = $this->M_rating->getUserDetails($id);
		$result['rating'] = $this->M_rating->getRatingData($id);
		$result['userRating'] = $this->M_rating->getUserRatingData($id);
		//print_r($result['userRating']);die();
		$result['locationsData'] = $this->M_user->getLocationsDetails($id); 
		//print_r($catResult['locationsData']);die();
		
		$this->M_user->updateUserNotification($id);
		$result['pageHeading'] = 'View User Rating';

		$this->load->view('v_header');
		$this->load->view('rating/v_userRatingView',$result);
		$this->load->view('v_footer');	
	}
