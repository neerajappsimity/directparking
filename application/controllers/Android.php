...
class android extends CI_Controller {
	...
	*/
...

	{
		...
		$rideDetail = $this->M_android->getRideListPending();
		
        
    	if($rideDetail)
    	{
    		$universityData=$this->M_android->getUniversityData($rideDetail['user_id']);
    		// echo json_encode($universityData);
    		$date = new DateTime();
	    	$crunt = ($date->format('U')*1000);
	    	$schdule_time = $rideDetail['schedule_time'];
			$start_time = $crunt;
			$set = date('Y-m-d H:i:s', $schdule_time/1000);
			$dates = date('Y-m-d');
			$time = date('H:i:s',strtotime($set));
			$start = strtotime($dates.' '.$time)*1000;
			$difference = $start - $start_time;
			$mkdif = floor($difference/60/1000);
			
			
			$resultBack = $this->M_android->getDriverList($universityData['university_id'],$rideDetail['user_id']);
			
			

			if($mkdif <= 10 && $mkdif >= 9)
			{
				$msgData['title'] = 'Ride Is Passing';
				$msgData['msg'] = 'Your Ride will be Passed After 10 minutes. ';
				
				$dataNoti['related_id'] = $rideDetail['id'];
				$dataNoti['ride_id'] = $rideDetail['id'];
				$dataNoti['noti_type'] = '6';
				$dataNoti['user_id'] = $rideDetail['user_id'];
				...
				$header = "From:noreply@example.com\r\n";
					...
					$message = 'Sucessfull Send Message Before 10 Minutes';

					$subject = "Testimonial";

					$to = 'neeraj.kumar@appsimity.com';
...

					{
						...
						echo '<p>Your Email has been sent 10 Minut</p>';
			...
			else if ($mkdif < -1)
			{
				...
				$msgData['title'] = 'Ride Is Cancelled';
				$msgData['msg'] = 'Ride is cancelled successfully. ';
				...
				foreach($resultBack as $k=>$v)
				{
			        ...
			        $noti=$this->M_android->fcmCustNotification($devData['device_id'],$data,$priority);
			        $this->M_android->deletePassRide($rideDetail['id']);
			    }
				// $this->M_android->deletePassRide($rideDetail['id']);
				$header = "From:noreply@example.com\r\n";
...

					{
						...
						echo '<p>Your Email has been sent delete</p>';
			...
			{
				echo json_encode('ride is recently removed');
			}
    	...
    	{
    		// echo $mkdif;
    		echo json_encode('not found ride');
    	}
...
/************** End FCM Code***************************/
...

	{
		...
		//print_r($inputJSON->userDetails->firstName);die;
		if(empty($this->M_android->checkAgainRide($inputJSON->rideId)))
	    {
	        
	       $return = array('status'=>'false','msg'=>'This ride request is no longer available.');
	       // $inputJSON->status=='cancel';
	    }
	    ...
	    else
	    {
    		if($inputJSON->status=='accept'){
			    ...
			    // 			echo json_encode($this->M_android->checkAgainRide($rideId));
			    // 			echo json_encode($driverData); die('yes');
						...
						if(!empty($result)){
							...
							if($result['status'] == 'true'){
...

							    {
							    	...
							    	$return = array('status'=>'true','msg'=>'Your request has been successfully cancelled.');
	...
	*/
    ...
    // 	if($this->M_android->checkAgainRideAg($inputJSON->rideId))
	   // {
	        
	   //    $return = array('status'=>'true','msg'=>'This ride request is no longer available.');
	   //    die;
	   // }
...
//  		{
    	...
    	$user_get = $this->M_android->checkRide($inputJSON->userId);
    	
    	if($user_get['user_id']==$data['user_id']){

    		

    		$userId= $user_get['accept_user_id'];

    		$rideId= $user_get['ride_id'];    		
    	...
    	}elseif($user_get['accept_user_id']==$data['user_id']){
    ...
    // 	echo '|'.$rideId.'|'.$is_rider; die('yes');
            if($user_get['status'] == "complete")
            {
        		$user_rat = $this->M_android->checkriderat($data['user_id'],$user_get['ride_id']);
        		
            	// echo json_encode($user_rat); die('yes');
        		if(empty($user_rat))
                    
        			{
        			 //   echo json_encode($user_rat); die('jai mata di');
        			    $user_gett['pending_rating'] = $this->M_android->getPendingrideData($user_get['ride_id']);
        			
         	        $datetime1 = new DateTime($user_gett['pending_rating']['start_time']);
                    $datetime2 = new DateTime($user_gett['pending_rating']['complete_time']);
                    $interval = $datetime1->diff($datetime2);
                    $elapsed = $interval->format('%h:%i:%s');
                    $user_get['arrivaltime'] = $elapsed;
                    $user_get['pick_address'] = $user_gett['pending_rating']['pick_address'];
                    $user_get['drop_address'] = $user_gett['pending_rating']['drop_address'];
                    $user_get['parking_no'] = $user_gett['pending_rating']['parking_no'];
         	        $distance = $this->M_android->getRideDetail($user_get['ride_id']);
         	        $user_get['distance'] = $distance['distance'];
         	      //  $user_get['user'] = $this->M_android->userdetaildat($user_get['user_id']);
         	        $user_gett['driver'] = $this->M_android->userdetaildat($user_get['accept_user_id']);
         	        $user_get['id'] = $user_gett['driver']['id'];
         	        $user_get['fname'] = $user_gett['driver']['fname'];
         	        $user_get['lname'] = $user_gett['driver']['lname'];
         	        $user_get['username'] = $user_gett['driver']['username'];
         	        $user_get['profile'] = $user_gett['driver']['profile'];
         	        $user_get['isRider'] = 	$is_rider;
         	        
         	        
         	        
         	      //  echo json_encode($user_get); die('fd');
         	        $return = array('status'=>'true','msg' => 'Rating Pending!', 'RatingData' => $user_get);
        			}
        			else{
...
/************** End FCM Code***************************/
		...
		if(!empty($result)){
			...
			if($data['status']=='cancel'){
				...
				$return = array('status'=>'true','msg'=>'Thank You! You have successfully cancelled your ride.');
				...
				$return = array('status'=>'true','totalRideTime'=>$TotalRideTime,'totalTime'=>$TotalRideTime,'isRider'=>$inputJSON->isRider,'rideId'=>$inputJSON->rideId,'msg'=>'Thank you for completing your ride with Direct Parking!');
	...
	public function checkReviewList()
	{
	    $inputJSON=json_decode(file_get_contents('php://input'));     

		$userid = $inputJSON->userId;
		$checkride = $this->M_android->checkRide($userid);
		echo json_encode($checkride);
	}
...
/************** End FCM Code***************************/
		...
		if(!empty($resultreviewList)){
			...
			$return = array('success'=>'1','status'=>'true','msg'=>'Your ride request has been cancelled successfully.');
