...
mail(orders_email, $adminSubject, $adminBody.$body, $header);// anchor email
	...
	//http://192.168.1.4/pumpkart/Android/getChecksum/?data=
	public function getChecksum()
	{
		$data_string = $_REQUEST['data'];
		$checkSum = getChecksumFromString($data_string,"E21VRY7ibUecSDxL");
		json_encode($checkSum);
	}
      
    //http://192.168.1.4/pumpkart/Android/verifyCheckSum/?checksumhash=&str=
	public function verifyCheckSum()
	{
		$checksumhash = $_REQUEST['checksumhash'];
		$str = $_REQUEST['str'];
		$key = 'E21VRY7ibUecSDxL';
		$result = verifychecksumFromstr_e($str,$key,$checksumhash);
		json_encode($result);
	}
