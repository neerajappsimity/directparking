...
class android extends CI_Controller {
	...
	public function userLogin()
	{
		...
		{
			if($result['is_pass_changed'] == 'D' || $result['is_pass_changed'] == 'Y'){
				...
				//'tax_name'=>'VAT'
			if(!empty($result['gst'])){
				$resultLogin= array('success'=>'1','user_id'=>$result['id'],'user_type'=>$result['user_type_id'], 'first_name'=>$result['fname'],'last_name'=>$result['lname'],'email'=>$result['email'],'mobile'=>$result['mobile'],'tax_type_id'=>'4','tax_name'=>'GST','tax_rate'=>'14.5','is_pass_changed'=>$result['is_pass_changed'],'gst'=>$result['gst']);
			}else{
				$resultLogin= array('success'=>'1','user_id'=>$result['id'],'user_type'=>$result['user_type_id'], 'first_name'=>$result['fname'],'last_name'=>$result['lname'],'email'=>$result['email'],'mobile'=>$result['mobile'],'tax_type_id'=>'1','tax_name'=>'VAT','tax_rate'=>'14.5','is_pass_changed'=>$result['is_pass_changed'],'gst'=>$result['gst']);
		...
		foreach($inputData->data as $v){
			...
			foreach ($cat as $Ckey => $Cvalue) {
				...
				if(!empty($result)){
					...
					$tempImage = 'http://pumpkart.in/admin/assets/image/graphs/'.$value['image'];
...
mail(orders_email, $adminSubject, $adminBody.$body, $header);// anchor email
		...
		$resultorder= array('success'=>'true','message'=>'Order placed successfully.','order_id'=>$resultOrderId,'pkart'=>$pkart, 'order_date'=>date('l d M, Y'));
	...
	public function verifyCoupon()
	{
		...
		foreach($inputJSON->product_details as $k=>$data)
		{
			...
			if($couponApplied==1){
				$response = array('success'=>'1','message'=>'Valid coupon code.','coupon_value'=>$result['coupon_value'],'coupon_value_type'=>$result['value_type'],'coupon_discount'=>$discount, 'coupon_id'=>$result['id'], 'data'=>$finalData);
			}else{
				$response = array('success'=>'0','message'=>'Coupon cannot be applied.');
			}
			...
			if(!isset($data['old_password'])){
				...
				if(empty($data['old_password'])){
					...
					if($userData['password'] == md5($data['old_password'])){
						...
						$return = array('status'=>'false','success'=>'0','message'=>'Old password do not match.');
					}
				}
	...
	public function getBanners()
	{
		/*if(!isset($_REQUEST['ver'])){
			...
			if($_REQUEST['ver'] == 'v2'){
				...
				$images['success'] = 0;
				$images['message'] = "Please update your application.";
			}
	...
	public function getDataSheet()
	{
		...
		foreach ($cat as $Ckey => $Cvalue) {
			...
			if(!empty($result)){
				foreach($result as $key => $value) {
				$image[] = 'http://pumpkart.in/admin/assets/image/graphs/'.$value['image'];
				}
			}
		}
