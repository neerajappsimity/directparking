						...
						<div class="portlet">
							<div class="portlet-title">
								...
								<div class="actions btn-set">
									...
									<button class="btn green"><i class="fa fa-check"></i> Place Order</button>
									<input type="hidden" class="user_id" name="userId">
	...
	$('#user').autocomplete({
				...
				success: function( data ) {
							 response( $.map( data, function( item ) {
								...
								return {
									label: item.fname+' '+item.lname+' ('+item.mobile+')',
									data: item
								}
							}));
						}
		...
		success: function (htmlStr)
			{
				...
				var str = '<tr class="rowPro" id="rowDisId_'+count+'"><td>'+htmlStr.name+'</td><td>'+htmlStr.sku+'</td><td><input type="text" class="form-control unitPrice_'+count+'" name="per_unit_price[]" value="'+htmlStr.price+'" onkeyup="updateTotal('+count+');"></td><td><input type="text" name="quantity[]" value="'+qty+'" class="form-control qty_'+count+'" onkeyup="updateTotal('+count+');"><input type="hidden" class="id _'+count+'" name="id[]" value="'+htmlStr.id+'"><input type="hidden" class="mrp _'+count+'" name="mrp[]" value="'+htmlStr.mrp+'"></td><td><input name="tax[]" type="text" readonly class="form-control tax_'+count+'" value="'+htmlStr.price_tax+'"><input name="tax_rate[]" type="hidden" class="taxRate_'+count+'" value="'+htmlStr.price_tax_rate+'"></td><td><input name="proTotal[]" type="text" readonly class="form-control total_'+count+' proTotal" value="'+((qty*htmlStr.price)+htmlStr.price_tax)+'"></td><td><button type="button" class="btn btn-danger" onclick="deleteProduct('+count+')">Remove</button></td></tr>';

				var tempTotalAmt = parseInt($('.totalAmount').val());
				var proTotal = ((qty*htmlStr.price)+htmlStr.price_tax);
...
function deleteProduct(val){
		...
		if (confirm("Are you sure, you want to delete?") == true) {
					  	...
					  	var proTotal = 0;
						$( ".proTotal" ).each(function( index ) {
	  						proTotal = proTotal+ parseInt($(this).html());
						});
