...
<div class="table_data" aria-labelledby="headingOne" role="tabpanel" id="chat">
	...
	<div class="portlet box grey">
						<div class="portlet-title">
							...
							<a class="btn blue" href="<?php echo site_url(); ?>student/exportData">Export</a>
							</div>
							<!--div class="actions">
						...
						<a href="<?php echo site_url(); ?>student/view?id=<?php echo base64_encode($row['id']); ?>" class="btn default green btn-full"><i class="fa fa-eye"></i> View</a>
<!-- 						<a href="<?php echo site_url(); ?>user/edit?id=<?php echo base64_encode($row['id']); ?>" class="btn default btn-full" style="background:#E8720B;color:#FFF;"><i class="fa fa-pencil"></i> Edit</a> -->
					...
					<?php if($row['enabled'] == 'Y'){?>
						<a id="rowIdStatus_<?php echo $row['id']; ?>" href="javascript:void(0)" onclick="changeStatus(<?php echo $row['id']; ?>, 'N')"><button type="button" class="btn btn-warning btn-full"><i class="fa fa-pencil"></i> Deactive</button></a>
					<?php }else{?>
						<a id="rowIdStatus_<?php echo $row['id']; ?>" href="javascript:void(0)" onclick="changeStatus(<?php echo $row['id']; ?>, 'Y')"><button type="button" class="btn btn-primary btn-full"><i class="fa fa-pencil"></i> Active</button></a>
					<?php }?>
...
<script>
			  ...
			  if(data == 'true')
			  {
				  ...
				  { 
					  /*$('#rowId_'+gId +'td:nth-child(3) a:nth-child(3)').html('Disable');*/ 
					   $('#rowIdStatus_'+id).html('<button type="button" class="btn btn-warning"><i class="fa fa-pencil"></i> Deactive &nbsp;</button>');
				  	   $('#rowIdStatus_'+id).attr("onclick", "changeStatus("+id+", 'N')");
				  }
				  else
				  {
					  /*$('#rowId_'+gId +'td:nth-child(3) a:nth-child(3)').html('Enable');*/
					  $('#rowIdStatus_'+id).html('<button type="button" class="btn btn-info"><i class="fa fa-pencil"></i>  Active &nbsp; &nbsp;</button>');
					  $('#rowIdStatus_'+id).attr("onclick", "changeStatus("+id+", 'Y')");
				  }
				  $('#message').html('<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Success!</strong> Student status changed.</div>');
			  }
			  else
			  { 
				  $('#message').html('<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Oops!</strong> Student status not changed.</div>');
	...
	function changeVerified(id, enabled)
	{
		...
		$.ajax({
			url: URL +"student/changeVerified", 
		  	type: "post", 
		  	data: dataToSend,     
		  	cache: false,
		    success: function(data) {
			console.log(data);
			  ...
			  { 
				  $('#message').html('<div class="alert alert-danger"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Oops!</strong> user status not changed.</div>');
...
alert('Your search string contains illegal characters.');
 ...
 window.location.href = window.location.href='<?php echo base_url().'student/'.$pageFunction.'/'; ?>' + $('#searchFORMHEAD_input').val()+ '<?php echo '?'.$_SERVER['QUERY_STRING'];?>';
