...
class m_android extends CI_Model {
	...
	public function checkAgainRide($rideId)
	{
	    $this->db->select('*');
	...
	public function checkRide($user_id)
    {
        $this->db->select('ride_id');
        $this->db->select('user_id');
        $this->db->select('accept_user_id');
        $this->db->select('status');
        $this->db->from('ride_history');
        $this->db->where('status', 'complete');
        $this->db->where('user_id', $user_id);
        $this->db->or_where('accept_user_id', $user_id);
    ...
    public function checkriderat($userid, $rideId)
    { 
        $this->db->select('*');
    ...
    public function getPendingrideData($rideId)
    {
        $this->db->select('pick_address');
        $this->db->select('drop_address');
        $this->db->select('parking_no');
        $this->db->select('start_time');
        $this->db->select('complete_time');
        $this->db->from('ride_booking');
        $this->db->where('id', $rideId);
        $query = $this->db->get();
        if($query->row_array() > 0){
	    ...
	    public function checkVerifiedEmail($data)
...

	{
		...
		$this->db->where('id',$data['university_id']);
		...
		//$this->db->where("domain LIKE '%".end($email)."%' ");
...

	{
		...
		$this->db->query("SET FOREIGN_KEY_CHECKS = 0");
			...
			if($this->db->update('tbl_users', $updatedData)){
	...
	public function addCarDetail($data)
...

	{
	   ...
	   // print_r($password);
		...
		$this->db->select('U.*,UV.name as university_name,UV.id as university_id');		
		...
		$this->db->join('universities as UV','U.university_id = UV.id');
	...
	public function checkDupRequest($data)
...

	{
		...
		$this->db->select('id as ride_id,user_id');
		...
		$this->db->where("DATE(created)",$data['schedule_date']);



		$this->db->where_in('status',array('start','accept','pending'));	



		$this->db->where_in('archive',array('N','Y'));		
	...
	public function checkAlreadtAccept($data)
...

	{
		...
		$this->db->where_in('status',array('accept','start'));	
		...
		public function AddRequestRide($data)
...

	{
		...
		public function getUniversityData($data)
...

	{
		...
		public function getDriver($universityData,$data)
...

	{
		...
		$this->db->select('id, email,device_id');
		...
		$this->db->where('university_id',$universityData);
	...
	public function getDriverList($universityData,$data)
...

    {
      ...
      //print_r($data);die();convert(n.created, getdate(), 3) 



      //$this->db->select('n.id,n.type,n.user_id,n.is_read, DATE_FORMAT( `n`.`created` , "%d-%m-%Y %H:%i" ) AS created,c.code,c.name as title,n.msg as desc');



      $this->db->select('n.id,n.ride_id,n.user_id,n.is_read, DATE_FORMAT( `n`.`created` , "%d/%m/%Y" ) AS created,nt.title,nt.description');
      ...
      $this->db->join('tbl_notifications_type as nt','nt.noti_type = n.noti_type'); 
	...
	public function getRideDetail($rideId)
...

	{
		...
		$this->db->select('*,ROUND(111.1111 *



       					DEGREES(ACOS(COS(RADIANS(pick_lat))
                ...
                * COS(RADIANS(pick_log) - RADIANS(drop_log))
       ...
       + SIN(RADIANS(pick_lat))
                ...
                * SIN(RADIANS(drop_lat)))) ,2)AS distance');
    ...
    public function userdetaildat($user)
    {
        $this->db->select('id');
	...
	public function getUserDetailData($rideId,$userId,$isRider)
...

	{
		...
		$this->db->select('usr.id as user_id,usr.fname,usr.lname,usr.profile');
		...
		if($isRider=='0'){
			...
			$this->db->join('ride_history as rh','rh.accept_user_id = usr.id'); 
			...
			$this->db->join('ride_history as rh','rh.user_id = usr.id'); 
		...
		$this->db->where('rh.ride_id', $rideId);



		$this->db->where('rh.status', 'complete');		
	...
	public function checkUnvId($userId)
	...
	public function getRideList($unvId,$userId)
...

	{
		...
		$this->db->select('round(AVG(rr.rating)) as rating,usr.id as user_id,usr.mobile,usr.profile,usr.fname,usr.lname,usr.email,rb.id as id,rb.schedule_date,rb.schedule_time,rb.pick_address,rb.pick_lat,rb.pick_log,rb.drop_address,rb.drop_address,rb.drop_lat,rb.drop_log,rb.status,rb.parking_no');



		$this->db->from('universities as unv');	



		$this->db->join('tbl_users as usr','unv.id= usr.university_id');		



		$this->db->join('ride_booking as rb','rb.user_id = usr.id');



		$this->db->join('review_rating as rr','rr.user_id = usr.id');	



		$this->db->where('unv.id', $unvId);
		...
		$this->db->where('rb.status', 'pending');
		...
		//$this->db->where('rb.created', date("Y-m-d"););
		...
		$this->db->where("rb.created LIKE '%".$date."%' ");
	...
	public function getRideListPending()

	{

		

		$cruntDate = date('Y-m-d');
		...
		$this->db->where_in('status',array('accept', 'pending'));



		$this->db->where("DATE(created)", $cruntDate);



		//$this->db->where('schedule_time>', $date->format('U'));
		...
		public function checkDupAcceptRide($data)
...

	{
		...
		$this->db->where('accept_user_id',$data);		



		$this->db->where("DATE(created)", $cuDate);



		$this->db->where_in('status',array('start','accept'));



		$this->db->where_in('archived',array('N', 'Y'));	
	...
	public function deletePassRide($data)
...
// 		$this->db->set('status', 'cancel');
		...
		$this->db->where('DATE(created)', $cruntDate);
		$this->db->delete('ride_booking');
		
		$this->db->where('ride_id',$data);
		$this->db->delete('ride_history');

// 		$this->db->update('ride_booking');
...
// 		}
		...
		// $a = $this->db->query("update ride_booking set status='cancel', where DATE(created)='".$cruntDate."', where id='".$data['id']."'");
	...
	public function deleteStuckRide($data)

	{
		...
		public function AddRideStatus($data)
...

	{
		...
		$this->db->insert('ride_history', $data);
		...
		function fcmCustNotification($to,$data,$priority){
	   ...
	   $header=array('Content-Type: application/json',
	     ...
	     "Authorization: key=AAAAnB5kS40:APA91bF0EAOt8bfWqygWblMq_LGeG04gO80LhwLoLfC3qOjGypJzyyhjc_vrMtW6I67ZX6v93OmdTzDmtQVSz8mznGQZdtNgJsPskPT1d01WqKqnzjYAPyO7Y4CErV4Wz38fLm0QxbgoOfue6JfMeFojI84lOnn4EQ");
	...
	public function addRideBookingStatus($data)
...

	{
			...
			$this->db->query("SET FOREIGN_KEY_CHECKS = 0");
				...
				if($this->db->update('ride_booking', $dataStatus)){
...

	{
		...
		//$this->db->where("created LIKE '%".$data['current_date']."%' ");
	...
	public function checkRatingExist($data)
		...
		$this->db->where('ride_id',$data['ride_id']);
		...
		if($this->db->update('review_rating', $data))
...
/*		public function getRider($data)
...

	{
		...
		$this->db->where('schedule_date',$data['current_date']);
...

	{
		...
		/*$this->db->select('id,user_id,accept_user_id,ride_id');
		...
		$this->db->or_where('accept_user_id',$data['user_id']);
		...
		$sql = "SELECT TIME(b.schedule_time) as timestamp, r.id, r.user_id, r.accept_user_id, r.ride_id, r.status FROM ride_history r inner join ride_booking b on b.id =r.ride_id  WHERE (r.user_id = '$uId' OR r.accept_user_id = '$uId') AND r.status IN('start', 'accept') AND DATE(r.created) = '$date'   ORDER BY id DESC Limit 0,1";
		...
		//select user_id,accept_user_id,ride_id from ride_history WHERE accept_user_id=92 OR user_id=92 order by id DESC limit 0,1 



		//$result = $this->db->query("user_id,accept_user_id,ride_id from ride_history WHERE accept_user_id='".$uId."' OR user_id='".$uId."'order by id DESC limit 0,1");
    ...
    public function getCancelRide($user)
    {
	...
	public function getPendingRide($data){
		...
		$sql = "SELECT * from ride_booking where user_id='$uId' AND status='pending' ORDER BY id DESC Limit 0,1";
...

	{
		...
		$this->db->select('user_id,ride_id');
		...
		$this->db->where('accept_user_id',$data['user_id']);		
...

	{
		...
		public function trackRide($userid,$rideid)
...

	{
			...
			$sql = "SELECT ride_booking.*,tbl_users.lat,tbl_users.lat,tbl_users.lng,tbl_users.bearing,tbl_users.id as driverid from ride_booking inner join ride_history on ride_history.ride_id=ride_booking.id inner join tbl_users on tbl_users.id=ride_history.accept_user_id where ride_booking.user_id='$userid' and ride_booking.id='$rideid' and ride_booking.status IN('accept','start')";
...

	{
       ...
       + SIN(RADIANS(pick_lat))
                ...
                * SIN(RADIANS(drop_lat)))) AS distance');
	...
	public function afterAcceptRiderStatus($data)
...

	{
		...
		//print_r($data);die;
			...
			$rideHistatus['status']=$data['status'];
			...
			$this->db->query("SET FOREIGN_KEY_CHECKS = 0");
				...
				if($this->db->update('ride_booking', $dataStatus)){
					...
					$this->db->where('ride_id', $Rid);			



					if($this->db->update('ride_history', $rideHistatus)){
		...
		public function checkRiderUserId($rideId)
...

	{
		...
		$this->db->select('accept_user_id as driver_id');
		...
		public function checkDriverUserId($rideId)
	...
	public function afterAcceptDriverStatus($data)
...

	{
			...
			$this->db->query("SET FOREIGN_KEY_CHECKS = 0");
				...
				$this->db->where('accept_user_id', $Uid);			
				...
				$result=$this->db->update('ride_history', $dataStatus);	



			if($dataStatus['status']=='cancel'){
				...
				$result=$this->db->update('ride_booking', $bookData);	
				...
				}elseif($dataStatus['status']=='start'){



				$bookData['status']='start';



				$bookData['start_time']= date('Y-m-d H:i:s');



				//print_r($bookData['start_time']);die;
				...
				}elseif($dataStatus['status']=='complete'){



				$bookData['status']='complete';



				$bookData['complete_time']= date('Y-m-d H:i:s');
				...
				}elseif($dataStatus['status']=='do not show'){



				$bookData['status']='do not show';
		...
		public function getTotalRideTime($rideId)
...

	{
		...
		$this->db->select('TIMEDIFF(complete_time,start_time) as totalTime');
...

	{
		...
		$this->db->select('u.id as userid,u.fname,u.lname,u.profile,rv.rating,rv.comment,rv.created');



		$this->db->from('review_rating as rv');	



		$this->db->join('tbl_users as u','u.id = rv.related_id');	



		$this->db->where('rv.is_verified','Y');		



		$this->db->where('rv.user_id',$data['user_id']);		



		$this->db->where('rv.archive','N');
	...
	public function riderRequestCancel($data)
...

	{
		...
		$this->db->where('id', $Rid);			
			...
			if($this->db->update('ride_booking', $cuStatus)){
				...
				if($this->db->update('ride_history', $cuStatus)){
		...
		public function getUserId($rideId)
	...
	public function getMyRideHistory($unvId,$userId)
...

	{
		...
		$this->db->select('usr.id as user_id,IF(rt.rating IS NULL,0,rt.rating) AS rating,usr.mobile,usr.profile,usr.fname,usr.lname,usr.email,rb.id as rideId,rb.schedule_date,rb.schedule_time,rb.pick_address,rb.pick_lat,rb.pick_log,rb.drop_address,rb.drop_address,rb.drop_lat,rb.drop_log,rb.status');



		$this->db->from('ride_booking as rb');	



		$this->db->join('tbl_users as usr','rb.user_id= usr.id');



		$this->db->join('review_rating as rt','rt.ride_id = rb.id','left');	



		$this->db->join('universities as unv','unv.id= usr.university_id');
		...
		//$this->db->where('rb.status', 'complete');



		$this->db->where_in('rb.status',array('complete','cancel'));
		...
		public function getMyTripRideHistory($unvId,$userId)
...

	{
		...
		$this->db->select('usr.id as user_id,IF(rt.rating IS NULL,0,round(rt.rating)) AS rating,usr.mobile,usr.profile,usr.fname,usr.lname,usr.email,rb.id as rideId,rb.schedule_date,rb.schedule_time,rb.pick_address,rb.pick_lat,rb.pick_log,rb.drop_address,rb.drop_address,rb.drop_lat,rb.drop_log,rb.status');
		...
		$this->db->join('ride_history as rh','rh.ride_id = rb.id');	
		...
		$this->db->join('tbl_users as usr','rh.user_id = usr.id');
		...
		$this->db->where('rh.accept_user_id', $userId);	
	...
	public function checkOldPassword($userId)
    ...
    {	
      ...
      //$this->db->or_where('password',$oldPassword);
...

	{
		...
		//$sql = "SELECT msg.*,u.profile FROM messages as msg INNER JOIN tbl_users as u ON u.id=msg.to_user_id WHERE to_user_id = '$toUserId' AND from_user_id = '$fromUserId' ORDER BY id DESC";
		...
		$sql="SELECT msg.from_user_id as fromUserId,msg.to_user_id as toUserId,msg.ride_id as rideId, msg.message,msg.created ,u.profile FROM messages as msg INNER JOIN tbl_users as u ON u.id=msg.from_user_id where msg.ride_id='$data'";
	...
	public function updateCarDetail($data)
...

	{
		...
		$this->db->query("SET FOREIGN_KEY_CHECKS = 0");
			...
			if($this->db->update('car_detail', $data)){
	...
	public function getDataSheet($data)
...

	{
		...
		$this->db->select('P.commodity_id,P.stock_available,P.quantity,P.id,P.name,P.description,P.short_description,P.mrp,P.is_best,B.name as brand_name, B.support as brand_support, PRICE.price as price, I.image as featured_image');
		...
		if(isset($filter['recom_application'])){



			$this->db->where("P.applications LIKE '%".ucfirst($filter['recom_application'])."%' ");
		...
		if(isset($filter['recom_fluid'])){



			$this->db->where("P.fluid_type LIKE '%".ucfirst($filter['recom_fluid'])."%' ");
		...
		if(isset($filter['recom_phase'])){



			$phase = strtolower($filter['recom_phase']);
			...
			$this->db->where("P.phase_id = ".$phaseId." ");
		...
		if(isset($filter['recom_liquid_handle'])){



			$this->db->where("P.source_water LIKE '%".ucfirst($filter['recom_liquid_handle'])."%' ");
...
/*
...

	{
		...
		$this->db->select('P.*,B.name as brand_name, B.support as brand_support,PH.name as phase_name,SH.name as solid, BR.name as bathrooms, PRICE.price as price,I.image as featured_image, BD.name as bore_diameter_name, TANK.name as tank_type, PC.category_id, C.parent_id as parent_id');
		...
		$this->db->join('tbl_categories as C','C.id = PC.category_id','left');
...

	{
		...
		$this->db->join('tbl_retailer_types as RT','RT.id = U.retailer_id','left');



		$this->db->join('tbl_firm_types as FT','FT.id = U.firm_id','left');
...

	{
		...
		$orderData['transaction_id'] = $inputJSON->txnid;



		$orderData['paytm_txn_status'] = $inputJSON->paytm_txn_status;
		...
		if(isset($inputJSON->order_id)){
			...
			$orderData['paytm_order_id'] = $inputJSON->order_id;
		...
		$orderData['delivery_fname'] = $inputJSON->delivery_fname;



		$orderData['delivery_lname'] = $inputJSON->delivery_lname;



		$orderData['delivery_user_type'] = $inputJSON->delivery_user_type;
		...
		$customer = $this->getCustomer($inputJSON->user_id);
        ...
        $orderData['bill_address_1'] = $customer['address_line_1'];



        $orderData['bill_address_2'] = $customer['address_line_2'];



        $orderData['bill_state'] = $customer['state_id'];



        $orderData['bill_city'] = $customer['city_id'];



        $orderData['bill_pincode'] = $customer['pincode'];
...

		{
			...
			if(isset($data->vatTax)){
				...
				$orderItemData['tax_amount'] = (($orderItemData['price_per_unit']*$orderItemData['quantity'])*$taxData['rate'])/100;
	...
	{        
		...
		$this->db->select('odr.delivery_user_type,odr.id as order_id,odr.net_amount ,odr.user_id,odr.created_date as order_created_date,net_amount, pm.name as payment_method, odr.shipping_charges, odr.tax_amount, os.order_status as order_status ');
...
/*	public function updateProfile($data)
...

	{
		...
		$this->db->from('tbl_tax_types as T');



		$this->db->where('T.tax_name','GST');
	...
	public function testSMS(){
		...
		//$msg = "Password Reset OTP :".$otp;
		 ...
		 $msg = "Dear ,\r\n Use  as one time password (OTP) to reset password on PUMPKART. Do not share this for security reasons. \r\n";
        ...
        $postUrl.= "UserName=".$username."&Password=".$password."&Type=Bulk&To=7696676269&Mask=".$mask."&Message=".urlencode($msg);
