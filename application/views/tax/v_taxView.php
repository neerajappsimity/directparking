	...
	<div class="portlet box grey">
						...
						<div style="float:right; margin-bottom:2%;">
							...
							<tr class="odd gradeX"  id="rowId_<?php echo $row['id']; ?>">
								...
								<td>
								<?php if(!empty($row['taxData'])){ ?>
									<table class="table table-striped table-bordered">
									<?php foreach($row['taxData'] as $keyT=>$valT){?>
									<tr>
								...
								<td>
								<a href="<?php echo site_url(); ?>tax/editTaxDetails?sid=<?php echo base64_encode($row['id']); ?>&tid=<?php echo $_GET['tid']; ?>" class="btn default purple"><i class="fa fa-pencil"></i> Edit</a>&nbsp;
								</td>
							</tr>
			<?php } }else{?>
			<tr>
...
alert('Your search string contains illegal characters.');
 ...
 window.location.href = window.location.href='<?php echo base_url().'tax/viewTax/'; ?>' + $('#searchFORMHEAD_input').val()+ '<?php echo '?'.$_SERVER['QUERY_STRING'];?>';
